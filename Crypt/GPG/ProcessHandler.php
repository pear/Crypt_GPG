...
class ProcessHandler
{
    ...
    public function setOperation($operation): void
    {
        ...
        if (!str_contains($operation, ' ')) {
            $op = trim($operation, '- ');
        } elseif (preg_match($regexp, $operation, $matches, PREG_OFFSET_CAPTURE)) {
            $op      = trim($matches[0][0], '-');
    ...
    public function handleStatus($line): void
    {
        ...
        switch ($tokens[0]) {
            ...
            case 'IMPORT_OK':
                ...
                } elseif (!in_array($tokens[2], $this->data['Import']['fingerprints'])) {
                    $this->data['Import']['fingerprints'][] = $tokens[2];
                }
        ...
        switch ($this->operation) {
            ...
            case 'list-keys':
                switch ($code) {
                    ...
                    default:
                        throw new Exceptions\Exception(
                            'Unknown error getting keys. ' . $note,
                            $code
                        );
                }
                break;
            ...
            case 'delete-secret-key':
                switch ($code) {
                    ...
                    default:
                        throw new Exceptions\Exception(
                            'Unknown error deleting key. ' . $note,
                            $code
                        );
                }
            ...
            case 'detach-sign':
                switch ($code) {
                    ...
                    default:
                        throw new Exceptions\Exception(
                            "Unknown error {$this->operation}ing data. $note",
                            $code
                        );
                }
                break; // @phpstan-ignore-line
            ...
            case 'gen-key':
                switch ($code) {
                    ...
                    default:
                        throw new Exceptions\Exception(
                            'Unknown error generating key-pair. ' . $note,
                            $code
                        );
                }
        }
    ...
    protected function getPin($key): string
    {
        ...
        if ($keyIdLength && !empty($_ENV['PINENTRY_USER_DATA'])) {
            ...
            foreach ($passphrases as $_keyId => $pass) {
                ...
                } elseif ($keyIdLength > $_keyIdLength) {
